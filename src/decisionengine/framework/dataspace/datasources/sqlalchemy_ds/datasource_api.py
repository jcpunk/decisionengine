...
class SQLAlchemyDS(ds.DataSource):
    ...
    def get_taskmanager(self, taskmanager_name, taskmanager_id=None):
        ...
        """
        with self.session() as session:
            query = session.query(db_schema.Taskmanager).filter(
                db_schema.Taskmanager.name == taskmanager_name
            )

            if taskmanager_id:
                query = query.filter(
                    db_schema.Taskmanager.taskmanager_id == taskmanager_id
                )
            else:
                max_id = (
                    session.query(
                        sql.func.max(db_schema.Taskmanager.sequence_id).label(
                            "sequence_id"
                        )
                    )
                ...
                query = query.filter(
                    db_schema.Taskmanager.sequence_id == max_id.c.sequence_id
                )
    ...
    def get_taskmanagers(self, taskmanager_name=None, start_time=None, end_time=None):
        ...
        """
        with self.session() as session:
            query = session.query(db_schema.Taskmanager).order_by(
                db_schema.Taskmanager.datestamp
            )
    ...
    def get_last_generation_id(self, taskmanager_name, taskmanager_id=None):
        ...
        with self.session() as session:
            query = (
                session.query(
                    sql.func.max(db_schema.Dataproduct.generation_id).label(
                        "generation_id"
                    )
                )
    ...
    def update(self, taskmanager_id, generation_id, key, value, header, metadata):
        ...
        my_header.expiration_time = header.get(
            "expiration_time", my_header.expiration_time
        )
        my_header.scheduled_create_time = header.get(
            "scheduled_create_time", my_header.scheduled_create_time
        )
        ...
        my_metadata.generation_time = metadata.get(
            "generation_time", my_metadata.generation_time
        )
        my_metadata.missed_update_count = metadata.get(
            "missed_update_count", my_metadata.missed_update_count
        )
    ...
    def delete_data_older_than(self, days):
        ...
        if days <= 0:
            ...
            raise ValueError(
                "Argument has to be positive, non zero integer. Supplied f{days}"
            )

        to_old = datetime.datetime.now() - datetime.timedelta(days=days)
        with self.session() as session:
            session.query(db_schema.Taskmanager).filter(
                db_schema.Taskmanager.datestamp < to_old
    ...
    def connect(self):
        ...
        Returns:
            None
        """
        self.engine = sqlalchemy.create_engine(**self.config_dict)
        self.logger.debug(f"Trying to connect as {self.engine.url.drivername}://{self.engine.url.username}@{self.engine.url.host}:{self.engine.url.port}/{self.engine.url.database}")
