...
"""
from sqlalchemy import Column, Index, ForeignKey, sql
...
class Taskmanager(Base):
    ...
    sequence_id = Column(
        SBigInteger, primary_key=True, index=True, unique=True, info="", comment=""
    )
    ...
    __table_args__ = (
        Index(
            "ix_taskmanager_taskmanager_id", "taskmanager_id", postgresql_using="hash"
        ),
    ...
    task_metadata = relationship(
        "Metadata", back_populates="taskmanager", cascade="delete"
    )
    task_dataproduct = relationship(
        "Dataproduct", back_populates="taskmanager", cascade="delete"
    )
...
class Header(Base):
    ...
    taskmanager = relationship(
        "Taskmanager", back_populates="task_header", passive_deletes=True
    )
...
class Metadata(Base):
    ...
    taskmanager = relationship(
        "Taskmanager", back_populates="task_metadata", passive_deletes=True
    )
...
class Dataproduct(Base):
    ...
    taskmanager = relationship(
        "Taskmanager", back_populates="task_dataproduct", passive_deletes=True
    )

    # Indexs, etc
    __table_args__ = (
        Index(
            "ix_dataproduct_taskmanager_id", "taskmanager_id", postgresql_using="hash"
        ),
        Index(
